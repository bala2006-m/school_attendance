...
class _AdminManagementDesktopState extends State<AdminManagementDesktop> {
  ...
  Future<void> _loadMenuOrder() async {
    ...
    final String? globalCatsJson = prefs.getString('admin_global_categories_order_${widget.schoolId}');
    final String? globalButtonsJson = prefs.getString('admin_global_buttons_order_${widget.schoolId}');
      ...
      if (response != null && response['status'] == 'success') {
        ...
        if (data != null) {
          ...
          if (updated) {
            ...
            final prefs = await SharedPreferences.getInstance();
            await prefs.setString('admin_global_categories_order_${widget.schoolId}', jsonEncode(cats));
            await prefs.setString('admin_global_buttons_order_${widget.schoolId}', jsonEncode(buttons));
          }
        }
      }
    } catch (e) {
  ...
  Future<void> _saveMenuOrder() async {
    ...
    await prefs.setString('admin_global_categories_order_${widget.schoolId}', jsonEncode(fullCats));
    await prefs.setString('admin_global_buttons_order_${widget.schoolId}', jsonEncode(fullButtons));
  ...
  @override
  void didUpdateWidget(covariant AdminManagementDesktop oldWidget) {
  ...
  void _initButtons() {
    ...
    if (_orderedCategories.isEmpty) {
      _orderedCategories = _allowedButtons.map((e) => e.category).toSet().toList();
    }
    
    // Ensure all categories have a button order
    for (var cat in _orderedCategories) {
      if (!_categoryButtonOrders.containsKey(cat) || _categoryButtonOrders[cat]!.isEmpty) {
        _categoryButtonOrders[cat] = _allowedButtons
                        ...
                        children: [
                          ElevatedButton.icon(
                            ...
                            style: ElevatedButton.styleFrom(
                              backgroundColor: isRearrangeMode ? Colors.green : Colors.blue.shade900,
  ...
  List<Widget> _buildAllCategories() {
    return _orderedCategories.where((category) {
      ...
      final buttons = _allowedButtons.where((b) => b.category == category).toList();
            ...
            isRearrangeMode
                ? SizedBox(
                  ...
                  child: ReorderableListView(
                    ...
                    onReorder: (oldIndex, newIndex) => _onButtonReorder(title, oldIndex, newIndex),
                    children:
                        buttons.map((data) {
                          return Container(
  ...
  void _onButtonReorder(String category, int oldIndex, int newIndex) {
    setState(() {
      ...
      final List<String> order = List<String>.from(_categoryButtonOrders[category] ?? []);
      if (order.isEmpty) {
        // Fallback to current button list
        final currentButtons = _allowedButtons.where((b) => b.category == category).toList();
