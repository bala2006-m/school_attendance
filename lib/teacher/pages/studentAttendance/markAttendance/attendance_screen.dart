...
class _AttendanceScreenState extends State<AttendanceScreen> {
  ...
  bool isSubmitted = false; // ✅ new flag
  ...
  Future<void> init() async {
    ...
    setState(() {
      isLoading = false;
      hasChanges = false;
      isSubmitted = false; // reset on fresh load
    });
  }
  ...
  void _checkForChanges() {
    ...
    setState(() {
      hasChanges = changed;
      if (changed) isSubmitted = false; // re-enable if edits made
    });
  }
  ...
  Future<void> _submitAttendance() async {
    ...
    setState(() {
      isSubmitting = false;
      if (anySuccess) {
        isSubmitted = true; // ✅ disable after success
        hasChanges = false;
      }
    });
      ...
      floatingActionButton:
          (isFnHoliday && isAnHoliday)
              ...
              : FloatingActionButton.extended(
                onPressed:
                    (isSubmitting ||
                            isSubmitted || // ✅ disable after submit
  ...
  Widget _buildSessionButton(String label, AttendanceSession type) {
    ...
    return ElevatedButton(
      ...
      onPressed: isHoliday ? null : () => setState(() => session = type),
      child: Text(
        label,
        style: TextStyle(
